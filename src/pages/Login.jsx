...
export default function Login() {
  ...
  const errorMessages = {
    ...
    EMAIL_OR_PAYROLLNUMBER_MISSING:
    ...
    <div className="min-h-screen bg-[#D9D0C1] flex items-center justify-center p-4">
      <div className="bg-white rounded-lg h-[32rem] w-[60rem] shadow-lg flex">
        <div className="bg-white w-1/2 flex flex-col items-center justify-center p-4 rounded-lg border-gray-200">
          <h1 className="font-sans font-bold text-start text-[25px] mb-9 self-start ml-6">
            Inicia sesión
          </h1>
          <Form
          ...
          >
            ...
            <Button
              type="submit"
              variant="flat"
              className="p-4 w-full bg-[#40251B] text-white font-bold"
            >
              {loading ? <Spinner color="warning" /> : "Inicia sesión"}
            </Button>
          </Form>
        </div>
        <div className="bg-[#40251B] w-1/2 flex flex-col items-center justify-center p-4 rounded-r-lg"></div>
      </div>
    </div>
  );
}
